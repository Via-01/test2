...
# ---------------------------------------------------------------------------
# View stock  (staff + hospital_admin can view)
# ---------------------------------------------------------------------------
...
def view_inventory_stock():
    db = next(get_db())
    try:
        # Aggregate stock per blood type
        items = db.query(
...
def execute_transfer():
    ...
    try:
        ...
        if source_unit == dest_unit:
            flash("Source and destination units cannot be the same.", 'error')
            return redirect(url_for('inventory.transfer_units_form'))
        ...
        if not source_inv:
            flash(f"No {blood_type_str} stock found in source unit '{source_unit}'.", 'error')
            return redirect(url_for('inventory.transfer_units_form'))

        if source_inv.unitsAvailable < quantity:
            flash(
                f"Insufficient stock: {source_inv.unitsAvailable} units available in "
                f"'{source_unit}', requested {quantity}.",
                'error',
            )
            return redirect(url_for('inventory.transfer_units_form'))
        ...
        db.commit()
        flash(
            f"Transferred {quantity} units of {blood_type_str} "
            f"from '{source_unit}' to '{dest_unit}'.",
            'success',
        )
        ...
        db.rollback()
        flash(f"Unexpected error during transfer: {e}", 'error')
        return redirect(url_for('inventory.transfer_units_form'))
    finally:
        db.close()